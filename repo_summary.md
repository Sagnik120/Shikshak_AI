...
Currently, only the **RAG** and **Avatar/Voice** pipelines are partially implemented. The entire AI Orchestration (state machine) and ML Core (evaluators) remain as design documents. An end-to-end flow is not yet runnable.
...
- `modules/ai_agent_orchestration/`: **IMPLEMENTED**. Contains core FSM and teaching agents.
- `modules/ml_core/`: **MISSING** (only `.gitkeep` and planned instructions).
...
*Status: PLANNED. Code is missing.*
...
| **ML Core** | MISSING | `modules/ml_core/src/` | Parsers, evaluators, misconception logic |
...
- **Implemented**: Basic unit tests exist for `rag` and `avatar_voice` (e.g., chunkers, parsers, TTS adapters).
- **Missing**: Cross-module E2E teaching session simulations and regression tests are non-existent due to missing backend/orchestration modules.
- **Risks**: The integration boundary between the unwritten Agent State Machine and the implemented Video/RAG services is completely untested.
...
- **P0**: Implement `ml_core` Answer Evaluator to support the teaching loop.
- **P1**: Build the `frontend` React UI (video player, interaction widgets).
...
- Examine `modules/rag/src/service.py` and `modules/avatar_voice/src/service.py` to understand the currently implemented APIs.
- Next steps involve implementing `modules/ml_core/src/` for evaluations, then scaffolding the FastAPI backend in `modules/backend/src/`.
