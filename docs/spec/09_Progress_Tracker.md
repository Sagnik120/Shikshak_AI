# 09_Progress_Tracker.md — High-Level Status Dashboard

## Protocol
This file defines the schema for `docs/progress.md`. Update `docs/progress.md` (not this file)
after every phase/sub-milestone using the template below.

## `docs/progress.md` Template
```
# Progress — <timestamp>
Overall Status: NOT STARTED | IN PROGRESS | STABLE | BLOCKED

## Phases (04_Phases.md)
- [ ] Phase 0 — Skeleton
- [ ] Phase 1 — Adapters & Ingestion
- [ ] Phase 2 — Planning & Retrieval
- [ ] Phase 3 — Explanation & Visual Selection
- [ ] Phase 4 — Video Generation
- [ ] Phase 5 — Interaction Loop
- [ ] Phase 6 — Evaluation & Adaptation
- [ ] Phase 7 — Assessment & Learner Profile
- [ ] Phase 8 — Frontend Polish & Multilingual
- [ ] Phase 9 — Documentation & Demo

## Mandatory Requirements Checklist (PS §17)
- [ ] Learning from uploaded material
- [ ] Topic-based teaching
- [ ] AI-generated lesson structure
- [ ] Personalized teaching
- [ ] Human-like teaching interaction
- [ ] Video-based AI Teacher presentation
- [ ] AI voice
- [ ] Human-like AI avatar
- [ ] Multilingual capability
- [ ] Student questioning & assessment
- [ ] Adaptive response to student performance
- [ ] Working application/prototype

## Known Issues / Blockers
- (none yet)
```
