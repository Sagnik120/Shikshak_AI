...
def main():
    ...
    if not api_key:
        ...
        print("    Usage: GEMINI_API_KEY=\"<key>\" python scripts/verify_live_gemini_quality.py")
        sys.exit(1)
    ...
    plan = planner.plan_from_topic(topic="Newton's Laws of Motion", constraints=constraints)
    ...
    segment = explainer.explain_node(
        node=first_node,
        language="en",
    ...
    question = questioner.generate_checkpoint(node=first_node, teaching_script=segment.script_text)
